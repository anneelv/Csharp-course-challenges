...
do
{
    ...
    switch (menuSelection)
    {
        ...
        case "2":
            ...
            string[] dogCharacteristicArray = new string[5];

            while (dogCharacteristic == "")
            {
                // #2 have user enter multiple comma separated characteristics to search for
                Console.WriteLine($"\r\nEnter dog characteristics to search for separated by commas (max. 5 characteristics)");
                readResult = Console.ReadLine();
                if (readResult != null)
                {
                    dogCharacteristic = readResult.ToLower().Trim();
                    dogCharacteristicArray = dogCharacteristic.Split(',');
                    // if I put 7 characteristics then the array will automatically expand?
                    Console.WriteLine("Number of characteristics: " + dogCharacteristicArray.Length);
                    foreach (string chars in dogCharacteristicArray)
                    {
            ...
            for (int i = 0; i < maxPets; i++)
            {
                ...
                if (ourAnimals[i, 1].Contains("dog"))
                {
                    ...
                    // #5 update "searching" message to show countdown 
                            ...
                            {
                                Console.Write($"\rsearching our dog {ourAnimals[i, 3]} for {chars} {icon} {j}");
                                Thread.Sleep(250);
                            } 
                        }
